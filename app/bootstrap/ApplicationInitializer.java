...
//			Club cmgc = entityManager.find(Club.class, 1);
			...
			players.add(scott);
			players.add(jeff);
			TeeTime teetime1 = new TeeTime(fivedaysfromnow.getTime(), champions.courses.get(0).teeSets.get(0), champions.courses.get(0), players, false, 4);
			teetime1.organizer = steve;
			for (TeeTimeParticipant participant : teetime1.participants) {
				participant.addScore(90, 90);
			}
			persist(teetime1);
			TeeTime teetime2 = new TeeTime(sixdaysfromnow.getTime(), champions.courses.get(0).teeSets.get(0), champions.courses.get(0), players, false, 4);
			teetime2.organizer = steve;
			for (TeeTimeParticipant participant : teetime2.participants) {
				participant.addScore(89, 89);
			}
			persist(teetime2);
			TeeTime teetime3 = new TeeTime(sevendaysfromnow.getTime(), champions.courses.get(0).teeSets.get(0), champions.courses.get(0), players, false, 4);
			teetime3.organizer = steve;
			for (TeeTimeParticipant participant : teetime3.participants) {
				participant.addScore(87, 87);
			}
			persist(teetime3);
			TeeTime teetime4 = new TeeTime(eightdaysfromnow.getTime(), champions.courses.get(0).teeSets.get(0), champions.courses.get(0), players, false, 4);
			...
			try {
				if (utx.getStatus() == Status.STATUS_ACTIVE) {
					try {
						...
						Logger.error("Error rolling back transaction", rbe);
					}
				}
			} catch (Exception se) {
			}
